...
function FormPage() {
  ...
  const getMappedSkinType = (oily: string, dry: string): string => {
    if (oily === "Likely" || oily === "Definitely") return "Oily";
    if (dry === "Likely" || dry === "Definitely") return "Dry";
    return "Combination";
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    data.append("skin_type", getMappedSkinType(isOily, isDry));
    data.append("age", "25");
    data.append("email", `user_${Date.now()}@dermasmart.app`);
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '3rem 2rem' }}>
        ...
        {error && (
          <div style={{
            ...
            color: '#ff6b6b', fontSize: '0.875rem', textAlign: 'center',
          }}>
            ⚠️ {error}
          </div>
        )}
